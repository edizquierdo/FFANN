...
class Microbial():
    def __init__(self, fitnessFunction, popsize, genesize, recombProb, mutatProb, demeSize, generations, boundaries):
        ...
        self.age = np.zeros((generations+1,popsize))
    ...
    def updateAge(self):  # Should make more efficient
        for i in range(self.popsize):
            if self.changed[i]:
                self.age[self.gen+1][i]=self.age[self.gen][i]+self.changed[i]
            else:
                self.age[self.gen+1][i]=0
